...
class ShapeModel():
    ...
    def load_model(self, path):
        self.model.load_state_dict(self.process_state_keys(self.parallel_to_cpu_state_dict(torch.load(path)['model_state_dict'])))
    ...
    def train(self):
        ...
        if self.config["ConditionalModel"]["cond_model"] == "EmbedImageCNN":
            cond_model = EmbedImageCNN(self.config["ConditionalModel"]["cond_sz"], **self.config["ConditionalModel"]["cond_model_args"])
        else:
    ...
    def test(self):
        betas = make_beta_schedule(schedule='sigmoid', n_timesteps=self.config["ConditionalModel"]["n_steps"], start=1e-5, end=1e-2)
        alphas = 1 - betas
        alphas_prod = torch.cumprod(alphas, dim=0)
